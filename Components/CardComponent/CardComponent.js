...
import { Card, siteSelectedCallback } from "react-bootstrap"
...
export default function CardComponent(props) {
    ...
    return (
        <div className={`p-1 ${styles.card} `}>
            <Card as="a" onClick={event =>  window.location.href='/'} style={{ cursor: "pointer" }} className={`box ${styles.cardComponent}`}>
                <Card.Img className={`${styles.cardTop}`}   variant="top" src={`${image_link}`} />
